...
Which is, I think, a bit easier to make sense of. But I can't quite figure out why they chose _this_ equation. Wouldn't this have also worked using $$\| (\b{x}_0 - \b{x}_k) - t (R\del f(\b{x}_k)) \|^2$$? Why'd they pick this one? The argument they give is: varying $$t$$ moves along the level set of $$f$$ (to first order) and gets you closer to the actual minimum value on the level set. Fine, I guess.

Also, it's not clear to me that two interleaved Newton iterators will actually converge to the right value, just because each one separately optimizes its individual value. Suppose $$\b{x}_k$$ was on the implicit surface $$f(\b{x}) = 0$$ but at a sharply curved point. Then the second "orthogonal" step could overshoot the root due to a second derivative, moving _off_ of the implicit surface, and then we'd have to move back onto it and potentially end up on the other side of the root and repeat the process. I dunno, maybe there's a proof that the error is bounded quadratically or something, but... I'm skeptical. It seems like it would fail if the surface was the graph of $$y = x^2$$ if $$\b{x}_0 = (0 - \e, 1)$$ and $$\b{x}_k$$ was just to the right of $$(0, 0)$$, for instance $$(\e, \e^2)$$ --- it would fly off to the left, right?
...
$$\frac{g_2(t)}{g'_2(t)} 
= \frac{[-R \del f(\b{x}_k)] \cdot [(\b{x}_0 - \b{x}_k) - t (R\del f(\b{x}_k))]}{\| R \del f(\b{x}_k) \|^2 } 
= \frac{-[(\b{x}_0 - \b{x}_k) - t (R\del f(\b{x}_k))]}{R \del f(\b{x}_k)}$$

I _guess_ that they're treating $$t= 0$$ as the current point, giving 

$$\frac{g_2(0)}{g'_2(0)}  =  - \frac{\b{x}_0 - \b{x}_k }{R \del f(\b{x}_k)}$$

(Keep in mind the notation for vector division). Now, this is a scalar, which make sense: it's the Newton iteration step for the function $$g_2(t)$$, which is scalar valued. I guess to make this into a vector you have to multiply it by $$R \del f(\b{x}_k)$$?

$$\begin{aligned}
\b{x}_{k+1} &= \b{x}_{k+1/2} - \frac{g_2(0)}{g'_2(0)} [R \del f(\b{x}_k)] \\
&= \b{x}_{k+1/2} + \frac{\b{x}_0 - \b{x}_k }{R \del f(\b{x}_k)} [R \del f(\b{x}_k)] \\
&= \b{x}_{k+1/2} + (\b{x}_0 - \b{x}_k)_{R \del f(\b{x}_k)} \\
\end{aligned}$$

But that's not what's in the paper. What happens is that they've rewritten that projection onto $$R \del f$$ into a rejection off of $$\del f$$, using a vector identity:

$$\b{a}_{R \b{b}} = \b{a} - \b{a}_{\b{b}}$$

(This identity works in $$\bb{R}^3$$ also if you replace $$\b{a}_{R \b{b}}$$ with $$\b{a}_{\perp \b{b}}$$, the perpendicular subspace to $$\b{b}$$.)

Giving 

$$
\b{x}_{k+1} = \b{x}_{k+1/2} + (\b{x}_0 - \b{x}_k) - (\b{x}_0 - \b{x}_k)_{\del f(\b{x}_k)} \\
$$
...
$$\begin{aligned}
\b{x}_{k + 1/2} &= \b{x}_k - \frac{f(\b{x}_k)}{\del f(\b{x}_k)} \\
\b{x}_{k+1} &= \b{x}_{k + \frac{1}{2}} - \frac{g_2(0)}{g_2'(0)} (R \del f(\b{x}_k)) \\
&= \b{x}_{k + \frac{1}{2}} + (\b{x}_0 - \b{x}_k)_{R \del f(\b{x}_k)} \\
&= \b{x}_{k + \frac{1}{2}} + (\b{x}_0 - \b{x}_k) - (\b{x}_0 - \b{x}_k)_{\del f(\b{x}_k)}
\end{aligned}
$$

Note: bit unsure on the signs in this derivation. I might have messed up a minus sign somewhere.

This equation should work in $$3d$$ also. Their $$g_2(t)$$ is bizarre and doesn't work in $$\bb{R}^3$$, but the iteration they got should work in $$\bb{R}^3$$ also because it doesn't privilege $$\b{k}$$.
