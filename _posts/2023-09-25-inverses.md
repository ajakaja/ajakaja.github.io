...
regardless of whether $$a$$ is "invertible". Here $$a_{\parallel}^{-1}$$ is a sort of "parallel inverse" (in some cases called the "pseudoinverse"), which is the invertible part of $$a$$, while $$a_{\perp}^{-1}$$ is an "orthogonal inverse" which null space of kernel of $$a$$, the objects that have $$ax = 0$$. It's probably not always true that a formula like this works, but it often does.
...
Every equation that we solve is an inverse problem. Finding the roots of a polynomial is inverting $$P(x) = 0$$. Computing an integral $$\int f(x) dx$$ is inverting $$dF(x) = f(x)$$. Solving a differential equation with boundary value constraints is inverting $$\begin{cases} Df = g \\ Bf = 0 \end{cases}$$. And mabe when we've fully wrapped our head around inverting linear problems in generality, we can extend the generic approach to non-linear problems as well? 

In either case I expect that a long list of tricks and theorems should fall out as fairly generic instantiations of the same general principle.

To proceed, we should move on from the observation that inverting $$ax = b$$ has this $$a_{\parallel}^{-1} (b) + a_{\parallel}^{-1}(0)$$ structure, to actually producing some inverses explicitly.