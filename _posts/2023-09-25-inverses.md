...
There's a lot more to say about those ideas, which I hope to investigate in a later article. For instance dividing by zero and producing degrees of freedom seems fun to play around with in other settings. Also, it's not clear that there's any good algorithm for these multivector representations anyway. Also, since _oriented_ projective geometry seems better in every other setting, it's probably also going to be advantageous, for some unknown reason, to treat all of this division as oriented as well. Also, we didn't consider _overspecified_ matrices at all, which are probably going to make even _more_ zeroes. Who knows!?
