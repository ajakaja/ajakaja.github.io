...
<small>(warning: this starts comprehensible and then gets _very_ not)</small>
...
In the familiar example of invertible $$A$$, this looks like

$$\begin{aligned}
\chi &= \star \^^{n} (A, -\b{b}) \\
&= \star (-\b{b} \^ A^{\^(n-1)}, A^{\^n}) \\
(\lambda \b{x}, \lambda) &= \star (-\b{b} \^ A^{\^(n-1)}, A^{\^n}) \\
\b{x} &= -\frac{\b{b} \^ A^{\^(n-1)}}{A^{\^n}} \\
&= \frac{\text{adj}(A) \b{b}}{\det (A)} \\
&= A^{-1} \b{b}
\end{aligned}$$

(...up to a minus sign or something, I haven't been keeping track. $$\text{adj}(A)$$ is the [adjugate matrix](https://en.wikipedia.org/wiki/Adjugate_matrix), you know, that matrix from intro linear algebra where every term is an $$(n-1)$$ minor determinant / something called a cofactor)
...
Moreover, the value we end up getting is definitely the same object as the Moore-Penrose pseudoinverse, in some notation. So that means that _this_

$$\b{x} = A^+ (\b{b}) + (I - A^+ A) \vec{\lambda}$$

is definitely another way of writing _this_

$$\b{x} = -\frac{\adj A}{\det A} \cdot \b{b}$$

We can sketch out a way of equating the two representations. We imagine rewriting $$\det A = A^{\^}_{\parallel} \^ A^{\^}_{\perp}$$, where $$A^{\^}$$, splitting it into two multivectors, $$A^{\parallel}$$ containing, say, $$k$$ orthogonal vectors of the row space and $$A_{\perp}$$ containing $$k$$ orthogonal vectors of the null space. Then the inverted parallel term $$(A_{\parallel}^{-1}) \b{b} = A^+ \b{b}$$ should produce $$k$$ solutions, and the inverted $$(0^{n-k} A^{\perp})^{-1} = (I - A^+ A) \vec{\lambda}^{n-k}$$ component should produce the $$n-k$$ degrees of freedom. _Evidently_ the inverse of zero is a "degree of freedom". Which makes a lot of sense.

Note: the only reason we moved _into_ projective coordinates was to deal with the problems of "dividing by zero" and "getting infinity" and the like in a way our math system could handle. But if our math system could _handle_ dividing by zero and producing degrees of freedom correctly on its own, then we wouldn't have needed any of the projective coordinate stuff. This suggests a general principle: **projective coordinates are being used to work around a problem in the underlying calculus**.


There's a lot more to say about those ideas, which I hope to investigate in a later article. For instance dividing by zero and producing degrees of freedom seems fun to play around with in other settings. Also, it's not clear that there's any good algorithm for these multivector representations anyway. Also, since _oriented_ projective geometry seems better in every other setting, it's probably also going to be advantageous, for some unknown reason, to treat all of this division as oriented as well. Who knows!?
...
The way we solved $$A \b{x} = \b{b}$$ in projective coordinates for matrices can give us ideas for solving $$D f = g$$ for differential operators, also. First we'll look at the concepts of degrees of freedom translate.

I needed to remind myself, first, of the rules for how many degrees of freedom there are in a differential equation solution. Obviously an $$N$$th-order differential equation can end up with integration constants:
...
Now look what happens when we add boundary conditions. We can include them as an additional term in a "homogenized" equation (in many cases, at least):
...
Then we might imagine that in some sense:

$$\lambda (f, 1) = Q^{-1}(0) = \star [\wedge^{n} Q]$$ as in the matrix case, although I don't have any idea how you compute $$\^n$$ of a matrix _of differnetial operators_. But if you could, it seems like you would get the correct form again:

$$f = Q_{\parallel}^{-1} (0) + Q_{\perp}^{-1} \cdot \Lambda$$
...
Of course, I wouldn't know where to start with making $$\begin{pmatrix} D & -g \\ B &  0 \end{pmatrix}^{-1}$$ mean something right now; that's a matrix of operators you're talking about! But, heck, give me the right non-commutative exterior algebra and maybe it just works? It's too clean not to.

(I expect we should call that object $$Q^{- \^1}$$, by the way: the "negative-$$1$$th wedge power of $$Q$$".)
...
I wouldn't be surprised, after all this, to find that there's some natural extension from this to solving some huge swathe of *non*-linear equations as well. Wouldn't that be neat? Why shouldn't division by a fifth-degree polynomial $$x = P_5^{-1}(0)$$ give an expression which is parameterized by coordinates in a contorted $$5$$-dimensional space that has full $$S_5$$ symmetry, whereas "all equations in the radicals", which is the inverse of a certain _type_ of polynomials, can be shown to not be able to express full $$S_5$$ symmetry (in particular its $$A_5$$ subgroup) and therefore there's [no quintic formula]((https://en.wikipedia.org/wiki/Abel%E2%80%93Ruffini_theorem))?

The basic procedure which seems to happen over and over is:

* To solve $$ax = b$$,
* Produce $$x = (b, a)^{-\^ (\dim b)}$$.
* This object is by definition the full solution space to $$ax = b$$, and is parameterized by free parameters which have the manifold structure of that solution space.
