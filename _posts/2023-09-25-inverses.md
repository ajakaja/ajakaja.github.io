...
Here's a notion I've had bouncing around for a while:

For a generic equation like $$ax = b$$ in which multiplication by $$a$$ is a linear operation, solutions seem to always be of the form 
...
\hat{f}(\b{k}) &= - \frac{\hat{g}(\b{k})}{k^2}  + \lambda_1 \delta(k) + \lambda_2 \frac{1}{k} \delta(k)
\end{aligned}$$

(For the last term, we need something that's $$O(k^{-1})$$. It seems like any term like $$\frac{\vec{\lambda} \cdot \b{k}}{k^2}$$ or even higher powers should work, but these are really awkward to take the Fourier transform of so I'm not _sure_ they work.)

Upon inverse-Fourier-transforming these two extra terms correspond to two types of terms that have $$\del^2 = 0$$: the first term is a constant $$\lambda_1$$, while the second gives a linear term like $$\vec{\lambda}_2 \cdot \b{x}$$.[^fourier2] Unsurprisingly, the Laplacian annihilates constant and linear terms, as expected.
[^fourier2]: At least... it should. I am trying to figure out how to actually do these Fourier transforms. Will report back later.
