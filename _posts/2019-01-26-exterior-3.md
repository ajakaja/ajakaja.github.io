...
For a taste of what it does, consider that we can write a bivector $$\b{b \^ c}$$ as a $$N \times N$$ antisymmetric matrix (literally, as $$\b{b \^ c} \ra \b{b} \o \b{c} - \b{c} \o \b{b}$$, which can be written with indees as $$b_i c_j - b_j c_i$$). Then the multiplication of this matrix by a vector is an interior product:

$$\b{a} \cdot (\b{b \^ c}) \equiv 
\begin{pmatrix} 0 & b_x c_y - b_y c_x & - b_x c_z + b_z c_x \\ 
- b_x c_y + b_y c_x & 0 & b_y c_z - b_z c_y \\
