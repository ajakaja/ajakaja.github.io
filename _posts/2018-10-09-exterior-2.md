...
However we define the inner product (or 'dot product'; I tend to use both names) on multivectors over $$\bb{R}^N$$, we're going to want it to act a lot like it does on vectors. Particularly, it seems like it should be true that

$$\| a \b{x \v y} + b \b{y \v z} + c \b{z \v x} \|^2 = a^2 + b^2 + c^2$$

ought to hold, since it's true for $$\| a \b{x} + b \b{y} + c \b{z} \|$$. More generally, for two multivectors in the same space, we should be able to sum over their components the same way we do for vectors with $$\< \b{u} , \b{v} \> \equiv \sum_{i \in V} u_i v_i$$:
...
And then extending this linearly to all elements of $$\v^k V$$. This gives an orthonormal basis on $$\v^k V$$, and the first thing we'll do is define the '$$k$$-lengths' of multivectors, in the same way that we compute the length of a vector $$\| \b{v} \| = \< \b{v} , \b{v} \>$$:
...
In these expressions, I'm going to be juggling multiple inner products at once. I'll denote them with subscripts: $$\<,\>_{\v}$$, $$\<,\>_{\o}$$, $$\<,\>_{V}$$. (I apologise for the similarity between $$\v$$ and $$V$$!) The types are:
