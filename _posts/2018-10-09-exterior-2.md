...
If you're working in $$\bb{R}^3$$, a lot of familiar updates are found after turning $$\^$$ into the cross product $$\times$$ using the  the [Hodge Star map](https://en.wikipedia.org/wiki/Hodge_star_operator) $$\star$$. We haven't studied this yet, but in $$\bb{R}^3$$ it usually means that formulas that hold for $$\^$$ also just hold for $$\star$$.

Transforming our bivector inner product, we immediately get the [Binet-Cauchy identity](https://en.wikipedia.org/wiki/Binet%E2%80%93Cauchy_identity):
...
This is because the total surface area bivector for a closed figure in $$\bb{R}^3$$ is $$0$$, so the surface area bivector of the opposing face is exactly $$-(a \b{x \^ y} + b \b{y \^ z} + c \b{z \^ x} )$$. 
