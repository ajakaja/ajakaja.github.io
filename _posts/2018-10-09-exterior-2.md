...
There is naturally a version of the law of cosines for any tetrahedron/ $$n$$_simplex with non-orthogonal sides as well. If $$\vec{c} = \vec{a} + \vec{b}$$ then (though it's often stated with $$c = b-a$$ instead):
...
We can easily expand $$\| a + b + c \|^2$$ linearly when $$a,b,c$$ are bivectors or anything else; the angles in the cosines become [angles between planes](https://en.wikipedia.org/wiki/Dihedral_angle) but the formula is otherwise the same.
