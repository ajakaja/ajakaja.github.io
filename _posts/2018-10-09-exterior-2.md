...
This is then extended linearly_ if either argument is a sum of multivectors. The left side of this is the inner product of two $$k$$-vectors (each are the wedge product of $$k$$ factors together); the right side is the determinant of a $$k \times k$$ matrix. For instance:
...
Let $$\text{Alt}$$ be the _Alternating Operator_, which takes a tensor product to its _total antisymmetrization_, eg $$\text{Alt}(\b{a \o b}) = \b{a \o b - b \o a}$$. For a tensor with $$N$$ factors, there are $$N!$$ components in the result.[^factorial]
...
Let $$A: U \ra V$$ and $$B: V \ra W$$ be linear transformations. Their composition $$\circ$$ has matrix representation:
...
Because $$A^{\^q} : \^^q U \ra \^^V$$ and $$B^{\^q} : \^^q V \ra \^^q W$$ are also linear transformations, their composition $$B^{\^q} \circ A^{\^q} : \^^q U \ra \^^W$$ also has a matrix representation:

$$(B^{\^q} A^{\^q})_I^K = \sum_{J \in \^^q V} (A^{\^q})_I^J (B^{\^q})_J^K = \< A^{\^q}_I, (B^{\^q})^K \>$$

Where here $$I,J,K$$ are indexes over the appropriate $$\^^q$$ spaces.
...
But this is just the determinant of a minor of $$(BA)_i^k$$. Namely, the one indexed by $$(I,K)$$, which means that:
