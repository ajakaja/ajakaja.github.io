...
The Fourier modes of the free electron wave function (ignoring spin) has this form with $$dS = p$$, the four-momentum: $$\psi = e^{i/\hbar p_\mu x^\mu}$$, with $$\| p \| = m$$, the rest mass. When an initial state is not a pure Fourier mode, we expand it as a sum of Fourier modes (for instance, if at $$t=0$$ we measure an electron at $$\vec{x} = 0$$, then the initial state is $$\psi(0, \vec{x}) = \delta(\vec{x}) = \int e^{i \vec{p} \cdot \vec{x}} d \vec{p}$$). When potentials are involved, $$dS$$ is modified; for instance the electromagnetic field enters as $$dS = p - qA$$. Depending on the field configuration we may no longer be able to easily integrate $$\int dS$$. For instance, if $$A$$ has a divergence, then the integral's result will depend on how many times $$\gamma$$ circles this pole. (If I understand correctly: in QFT this corresponds to how many times two particles interact. Which is wild.)

The exact values of $$\psi$$ (up to phase) and $$S$$ (up to a constant) are not physically observable; we only ever see their derivatives. The exact value of $$\| \p S \|$$ is found to be the rest mass $$m$$.
