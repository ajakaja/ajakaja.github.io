...
Conventions: $$c = 1$$, $$g_{\mu \nu} = diag(-, +, +, +)$$, because this keeps the operators in the form we expect. I like to write $$S_{\vec{x}}$$ for $$\nabla S$$.
...
$$\psi(t + \Delta t) = e^{i/\hbar \hat{H} \Delta t} \psi(t) = e^{\Delta t  \p_t} \psi(t) = e^{i/\hbar (\vec{p} \cdot \vec{x} - E(t + \Delta t))}$$
...
When potentials are involved, $$dS$$ is modified. The electromagnetic field, for instance, enters as $$dS = p - qA$$. Depending on the field configuration we may no longer be able to easily integrate $$\int dS$$: if $$A$$ includes a current, then it contains a 'line' of divergence, and the path integral's result will depend on how many times $$\Gamma$$ circles this pole in spacetime. This causes the path integral to give _different_ values based on the choice of path. Summing over these paths corresponds in QFT to summing over the number of photons that are exchanged (I think. Will work it out in detail when I get to QFT).
...
**Spin-$$1$$**: A _vector_-valued wave function $$\vec{\psi} = (\psi_x, \psi_y, \psi_z)$$, where the terms transform according to physical rotations, is a spin-1 object. To consider its angular momentum we diagonalize $$\hat{L}_z$$ by changing bases to 

$$(\psi_{+1}, \psi_0, \psi_{-1}) = (\psi_x + i \psi_y, \psi_z, \psi_x - i \psi_y)$$

These are known as [spherical tensors](https://en.wikipedia.org/wiki/Tensor_operator#Spherical_vector_operators). These terms locally look like $$e^{i/\hbar (l_z \phi + f(\theta) - Et)}$$ for $$l_z \in (+1, 0, -1)$$. (Apparently vector-valued wave functions have to have no radial variation, though, or this decomposition wouldn't work.) The $$0$$ component of $$L_z$$ spin corresponds to a vector-valued wave function pointing only in the $$z$$ direction. $$\pm 1$$ components correspond to having $$x$$ or $$y$$ components, with the sign determined by their relative phase.

Note what it means to have spin $$1$$: it's not that it fixes the _value_ of the angular momentum; rather, it specifies the different ways that the angular momentum can transform under rotation. The three choices determine whether $$\vec{\psi}$$ is in the $$z$$ direction $$(l_z = 0)$$ or whether it has a positive or negative 'rotational' components in the $$xy$$ plane ($$l_z = \pm 1$$). Particularly, having angular momentum $$l_z = +1$$ means that the $$y$$ component is advanced in phase compared to the $$x$$ component -- this is why the 'ladder' operator $$\hat{L}_+ = \hat{L}_x + i \hat{L}_y$$ serves to increase the angular momentum, because it includes a factor of $$e^{i \phi}$$:
...
Consider some arbitrary unit-vector function $$\vec{\psi} = (\sin \theta \cos \phi, \sin \theta \sin \phi, \cos \theta)$$. Apparently the angular-momentum states are:

$$
\begin{aligned}
(\psi_{+1}, \psi_0, \psi_{-1}) &= (\sin \theta (\cos \phi + i \sin \phi), \cos \theta, \sin \theta (\cos \phi - i \sin \phi)) \\ 
&= (\sin \theta \, e^{i \phi}, \cos \theta, \sin \theta \, e^{-i \phi})
\end{aligned}$$

Clearly $$\hat{L}_z (\psi_{+1}, \psi_0, \psi_{-1}) = (+1 \psi_{+1}, 0 \psi_{0}, -1 \psi_{-1})$$

Now take a look at the raising operator $$L_+$$:
...
By the way, photons are spin-1 particles, but to be in the $$l_z = 0$$ state would mean require that there be a rest frame where their vector points the same way as their velocity -- and they have no rest frame, so the $$l_z$$ value is forbidden. The remaining $$\pm 1$$ states correspond to photon polarizations.


**The Schrödinger Equation**: We can write $$ S_{\vec{x}}^2 - S_t^2  = - m^2$$ as $$S_t = \sqrt{m^2 + S_x^2} = m \sqrt{1 + \frac{S_x^2}{m^2}}$$ and expand as a Taylor series (note that $$\| S_x/m \| = \| p / m \| \ll 1$$) to get:
...
$$\int_i^f dS - q(A_\mu dx^\mu + d \Lambda) = S_i^f  - q \Lambda_i^f - q \int_i^f A_\mu dx^\mu$$
