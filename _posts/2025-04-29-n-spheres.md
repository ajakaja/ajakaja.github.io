...
We could work that out, but it's easier to defer to the Wikipedia for [beta functions](https://en.wikipedia.org/wiki/Beta_function). With $$z_1 = \frac{p}{2}+1$$ and $$z_2 = \frac{q}{2}$$:
...
This factor of $$n$$ is not specific to spheres. It is also there in tetrahedrons, or their $$n$$-dimensional generalization, called an $$n$$-simplex. If the base of an $$n$$-simplex $$P = (p_0 p_1 \ldots p_n)$$ is an $$(n-1)$$-simplex with area $$A$$ and the "height" $$h$$ is the distance from $$B$$ to the final point, then 
...
A cone is not, however, the circular equivalent of a tetrahedron. That would be something like this:

{% include image.html filename="2025-04-29-n-spheres/cone-2.png" width="250px" %}

The base of the cone is not self-similar in the same way: it doesn't have an obvious $$\frac{1}{2} bh$$ formula like a triangle. This is (presumably!) related to why the formulas for the volumes/areas of $$n$$-spheres do not have $$\frac{1}{n!}$$ in front of them.

The area of the cone is $$\Omega r^2$$, but it would be nice to see that in terms of numbers we know how to actually measure. Here's a diagram:
