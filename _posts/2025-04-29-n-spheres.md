...
edit: I ended up writing a series of sequels to this while trying to chase down exactly what $$(-\frac{1}{2})! = \sqrt{\pi}$$ could mean. So now this is part one:

1. Investigations on n-Spheres (you are here)
...
Here $$S_{n}$$ is a surface area of the $$n$$-sphere of radius $$r$$ (also called $$S^n$$, with a superscript) and $$V_n$$ is the volume of the $$n$$-ball $$B^n$$ which is the interior of $$S_{n-1}$$.

By the way, I don't really care for the "sphere" vs "ball" distinction that is common in mathematics, since I would use the word "sphere" colloquially to describe both. I like to just say they're the area and volume of the same object, the $$n$$-sphere. So most of the time I will not use the word ball. To be technically accurate, I am talking about the $$(n-1)$$-volumes of the $$(n-1)$$-spheres, and the associated $$n$$-volumes of their interiors, the $$n$$-balls. I will mostly try to use the terms correctly but... probably not consistently.
...
The most straightforward way is to compute a big integral: to find the area/volume of an $$n$$-sphere, integrate over its surface/interior. This is fairly straightforward, if tedious.
...
Higher dimensions have more and more copies of $$\sin^k (\phi_i)$$ on each successive angular variable, and thankfully, the big multivariable integral splits into a product into a product of single-variable integrals because none of the variables are shared. For instance
...
and when you work out the general case you get the formulas for $$S_n$$ (and $$V_n$$ if you integrate over $$r$$ as well).
...
What's really going on when we compute $$V_n$$ from $$V_{n-1}$$ is that, for every element on the line $$z \in (-r, r)$$, we are accumulating a copy of $$V_{n-1}$$ (with varying radius). But $$(-r, r)$$ is itself a copy of $$V_1$$, so the integral really has the form

$$V_n(r) = \int_0^{\pi/2} V_{n-1}(r \sin \phi_n) d V_1(z)$$

Where the integral is over volume of the $$1$$-sphere. In fact this can be done with more than just $$n = (n-1) \oplus (1)$$. An $$n = p + q$$ sphere can be seen as a "product" of a $$p$$-sphere and a $$q$$-sphere, like this:
...
For example: the volume of a sphere $$V_3$$ may be computed with $$p=2, q=1$$
...
Which corresponds to summing up a circle for each value of $$z = r_q$$, as above. (Note, the reason the the integration bounds are $$(0, \pi/2)$$ is because we're integrating over each possible radius inside $$V_1(r_q)$$, and at each radius there is a _pair_ of points, with "volume" $$S_0(r_q) = 2$$. The factor of $$2$$ takes care of adding the upper and lower hemispheres together.)
...
And I'll wwork it out explicitly to make sure I am not totally lost.
...
We could hack that out manually, but it's easier to defer to the Wikipedia for [beta functions](https://en.wikipedia.org/wiki/Beta_function). With $$z_1 = \frac{p}{2}+1$$ and $$z_2 = \frac{q}{2}$$ we get
