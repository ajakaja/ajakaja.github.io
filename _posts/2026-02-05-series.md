...
Which is my favorite function (because it's so mysterious). This form converges for $$\| x \|<1$$. If you want a version that works for $$\| x \|>1$$ you can use the expansion around $$x=\infty$$ instead. Simply write $$y=1/x$$ and expand everything in terms of $$y$$ instead:
...
Now here are five ways of thinking about where the Taylor series "comes from". Probably there is some insight to be gleaned from the fact that they are all equivalent.
...
Consider the case of (my favorite function) $$\frac{1}{1-x}$$. The Taylor series around $$x=0$$ is
...
The region of convergence is the 'disc' $$\| 1/x \| < 1$$, that is, $$\| x \| > 1$$ (I guess you can't really describe that as a "radius" of convergence, unless you want to say that it's $$\infty-1$$...). It works for all the values that (1) doesn't work for, like $$x=2$$:
...
One might interpret this to mean that although the result is not a number, it still 'contains' the data $$-1$$ somehow, maybe in a form like $$-1 + O(\infty)$$? But really it is not that hard to understand. The simple explanation is this:

Write $$S_0(x) = 1 + x + x^2 + \ldots$$ and $$S_{\infty}(x) = -\frac{1}{x} - \frac{1}{x^2} - \ldots$$. It is not actually the case that $$S_0(x)$$ is the multiplicative inverse of $$(1-x)$$. This is clear if you write it as the limit of a finite sum, $$S_0(x) = 1 + x + x^2 + \ldots + x^N$$, as $$N \ra \infty$$. Then

$$(1-x) S_0(x) = 1 - x^{N+1}$$

In the case where $$\lim_{N \ra \infty} x^{N} = 0$$ this works out to give the right answer. Otherwise, as in the case of $$S_0(2) = (1 - 2^{N+1})/(1-2) = 2^{N+1} - 1$$, it does not. But that is why it has the "value" $$-1$$ under the divergent-sum techniques: because they are simply ignoring the $$x^N$$ term entirely, even though it contributes to the result.
...
And these are both exact. As are the expansions around other points, such as $$x=k$$

$$
\begin{aligned}
\frac{1}{1-x} &= \frac{1}{1-k} \frac{1}{1-\frac{x-k}{1-k}} \\
&= \frac{1}{1-k}[1 +(\frac{x-k}{1-k}) + (\frac{x-k}{1-k})^2 + \ldots + (\frac{x-k}{1-k})^{N}] + \frac{1}{1-x} (\frac{x-k}{1-k})^{N+1}\\
&= S_k(x) + \frac{1}{1-x} (\frac{x-k}{1-k})^{N+1}
\end{aligned}
$$

(This sort of thing is clear if you go through the derivation of Taylor series rigorously, because you always track the remainder term. But I feel like people forget that it completely explains what's going on with divergent series as well.)[^divergent]

[^divergent]: Implicit in this way of thinking is the idea that you can generally factor objects into their functional dependence on an ambient variable (in this case $$N$$), and the result is usually meaningful. That's why the divergent-series sum techniques mostly work: because they're operating on the $$N$$-free part on its own. The same idea shows up in a lot of places and is, I think, generally under-appreciated. It's the idea behind the [entropy of a continuum]({% post_url 2018-02-23-entropy-1 %}), in which you separate out the part of the entropy which depends on your fine-graining of space from the constant part. It's also implied (I believe) in the concepts of [mixed volumes](https://en.wikipedia.org/wiki/Mixed_volume) and [numerosity](https://en.wikipedia.org/wiki/Numerosity_(mathematics)), although I haven't had the chance to figure out that connection in detail.
...
Whether or not you want to interpret that literally, this means that at least in infinite-series land, multiplication by $$(1-x)$$ is a zero divisor: there exists a nonzero divergent series $$f(x)$$ with $$(1-x)f(x) = 0$$. This means that any "inverse" of multiplication by $$(1-x)$$ has a free term: if $$S(x) = \frac{1}{1-x}$$ then $$S(x) + \lambda f(x)$$ is also a solution. This is sort of the sense in which $$S_0(x)$$ and $$S_{\infty}(x)$$ are "both" solutions to $$(1-x)^{-1}$$, if you ignore their $$N$$-dependence: they differ by a factor of $$f(x)$$. That arithmetic is actually illegitimate when you track the limits at infinity, but when it's not, it's a way of interpreting what's going on when multiple divergent series solve the same equation.
