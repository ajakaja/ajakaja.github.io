...
Here are all the main ways of thinking about where the Taylor series formulas (1) and (2) "come from". Probably there is some insight to be gleaned from the fact that they are all equivalent.
...
That's all I've got for now. I'll mention in passing that all of the above also gives one of the versions (among many) of the [fractional calculi](https://en.wikipedia.org/wiki/Fractional_calculus) when you allow yourself to do fractional combinatorics on the $$\p_x^k$$ operator. For instance Binomial-expanding/Taylor-expanding $$\p_x^{1/2}$$ gives a working version of a half-derivative that has $$[(\p_x)^{1/2}]^{\circ 2} = \p_x$$. Good luck figuring out what it means, though.
