...
There used to be a section here about divergent series summation techniques, but I decided to remove it to its [own article]([Exact Divergent Series]({% post_url 2018-11-01-summations %})) so that I could expand on it.

The short version is that

$$S_0(x) = 1+x+x^2+x^3 = \frac{1}{1-x}$$

should actually be thought of as

$$S_0(x) = 1 + x + x^2 + \ldots + x^n = \frac{1}{1-x} - \frac{x^{N+1}}{1-x}$$

which works always, and most of the divergent series trickery people do consists of finding ways to pretend like that second term doesn't exist so they can magically compute $$S_0(2) = -1$$.
