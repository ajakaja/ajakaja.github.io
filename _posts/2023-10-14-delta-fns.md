...
> $$e^{-ikx}$$ is an orthogonal basis for frequency-space. We write $$f$$ in this basis $$\hat{f}(k)$$ by projecting $$f(x)$$ onto each component of basis by taking an inner product $$\< e^{ikx}, f(x) \> = \int f(x) e^{-ikx} dx$$, where $$e^{-ikx}$$ is the conjugate of $$e^{ikx}$$ used in the inner product $$\< g, f \> = \int g^{*}(x) f(x) \d x$$
...
\begin{aligned}
\sgn(x) &= \sgn(x) x^0\\
\int \sgn(x) dx &= \sgn(x) x = \| x \| \\ 
\int \int \sgn(x) dx &= \sgn(x) \frac{x^2}{2} = \frac{x \| x \| }{2} \\ 
...
For example, you can't do calculus on $$\theta$$ in the normal way. Consider

$$\theta^k(x) = \theta(x)$$

The product rule does not work:
...
The problem is that you can't evaluate this linear approximation and expect to get a meaningful answer. $$\delta(x) \d x$$ only gives you the $$\pm 1$$ from crossing zero if you integrate over it. Which is to say, the $$+ \delta(x) \d x$$ has to be interpreted not as multiplication but as integration:
...
Let's write $$1_{dx}$$, a "signed indicator function", for this integral:

$$1_{dx} = \int_x^{x + dx} \delta(y) \d y = \begin{cases}
0 & x < 0, x + dx < 0 \\
+1 & x < 0, x + dx > 0 \\
-1 & x > 0, x + dx < 0 \\
0 & x > 0, x + dx > 0 
...
Also we'll abbreviate $$\theta_x$$ for $$\theta(x)$$ from now on. Then this _is_ true:

$$\theta_{x + dx} = \theta_x + 1_{dx}$$
...
&= \begin{cases}
0 = (0 + 0)^2 = 0^2 + 2 (0) (0) + 0^2       & x < 0, x + dx < 0 \\
1 = (0 + 1)^2 = 0^2 + 2 (0) (1) + 1^2       & x < 0, x + dx > 0 \\
0 = (1 - 1)^2 = 1^2 + 2 (1) (-1) + (-1)^2   & x > 0, x + dx < 0 \\
1 = (1 + 0)^2 = 1^2 + 2 (1) (0) + (0)^2 1   & x > 0, x + dx > 0 \\
\end{cases}
\end{aligned}
$$
...
This cleared things up for me. I vaguely knew that $$\p (\theta^k) = \p (\theta)$$ didn't work in the normal way, but I'm happier understanding exactly why: it is because $$\theta(x + dx) = \theta(x) + \delta(x) \d x$$ is not valid, and when write it as $$\theta_x + 1_{dx}$$, the latter term is not proportional to $$\| dx \|$$, and therefore the higher-order terms in the linear "approximation" to $$\theta_{x + dx}^k = (\theta_x + 1_{dx})^k$$ are not of vanishing magnitude and therefore must be preserved in the series expansion.
...
It's fun to find an odd representation like this and then try to make the rules of algebra and calculus work out on it. At some level, they _must_ work, so it usually leads you to natural extensions of the rules to objects they are not normally defined on (e.g. in the previous section).
...
Or at least, it should be possible to talk about their derivatives and Fourier transforms in _some_ sense. I guess. I dunno. It's worth a try at least.
