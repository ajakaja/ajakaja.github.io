...
Okay, maybe that's cheating. But there is something elegant about it. Can it be that  that $$\F(x \delta(x)) = k \delta(k)$$ (up to a constant maybe)?. And it's interesting that $$x \delta(x)$$ is a "pair of important objects in the theory"---  it has a certain [Parseval](https://en.wikipedia.org/wiki/Parseval%27s_theorem) nature to it. (Well really there are three objects in the theory: $$\{ x, \delta(x), \p_x \}$$... which all obey different rules. But in some sense you can construct $$\p_x$$ out of $$\delta(x)/x$$ so maybe it's really just two still.)
...
## 4. $$\delta(g(x))$$ becomes a sun over poles
