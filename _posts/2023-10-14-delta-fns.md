...
The fact that this works seems miraculous, but in fact it is just a heavily obfuscated version of $$f(0) = \iint \delta(z, \bar{z}) f(z) \, d \bar{z} \^ dz$$ which makes it obvious. Basically it [turns out]({% post_url 2020-08-10-complex-analysis %}) that $$\p_{\bar{z}}\frac{1}{z} = \delta(z, \bar{z})$$ and then you apply Stoke's theorem.
...
It is easier to see what this does for $$n < 0$$ if we use the Stoke's theorem representation: $$f^{(-n)}(0) = \iint \frac{z^{n}}{n!} \delta(z, \bar{z}) f(z) \, d \bar{z} \^ dz$$. Evidently they are extracting the constant terms of $$z^n f(z)$$, aka, the coefficients of the $$f_{-n} \frac{n! }{z^n}$$ terms in the Laurent series.[^unsure]

[^unsure]: I haven't closely checked these equations, although I mean to do so eventually. There might be missing coefficients or something. When I worked this out in a previous article I only did so for $$n=1$$.
...
Anyway, I mention all this to say that, imo, $$f^{(n)}(0)$$ is a perfectly useful concept for both positive and negative $$n$$. For positive, it's the $$n$$th derivative. For negative, it's the $$n$$th residue / integral / moment / whatever. Since they're contour integrals they're not _quite_ the same, but there is a close relationship. If $$\lim_{\| z \| \ra \infty} f(z) = 0$$ then we can close the contour at infinity without adding any value to the integral so they should be literally the same.
