...
Technically, $$x^n \delta(x)$$ for $$n$$ a positive integer is equal to zero, since it's integral against a test function is zero. But I'm not so sure. Test functions seem a bit made up, anyway. seems to mel ike $$x^n \delta(x)$$ would not equal zero _if_ integrated against a function with a pole at zero of order at least $$n$$, and that might be useful.
