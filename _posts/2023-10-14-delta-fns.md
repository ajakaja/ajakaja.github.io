...
Etc. I write this all out because it is easy to get confused by the factorials in there. Note that if $$\delta(x)$$ is replaced with something like $$\delta(x - a)$$, then all those denominators become $$\frac{1}{-(x-a)^n}$$.
...
Assuming that, once again, all the powers of zero other than $$0^0 = 1$$ "cancel out" somehow. So I would argue that $$\frac{x^n}{n!} \delta(x)$$ is extracting _residues_ the same way that $$\frac{n!}{x^n} \delta(x)$$ extracts _derivatives_. It's very nicely symmetric, wouldn't you agree?

To argue that $$x \delta = 0$$ is to argue that you're only concerned with finite values of integrals, which is fine... but not good enough. I believe that this stuff actually comes up a lot more than anyone thinks, albeit in very obfuscated forms, and I'm, I suppose, determined to get to the bottom of it.
...
but it's extracted in a much more intuitive way. I have [written before]({% post_url 2020-08-10-complex-analysis %}) about how the Cauchy integral formula works. The short version is that if you apply Stoke's theorm it turns into $$\iint \delta(\bar{z}, z) \d \bar{z} \^ \d z$$, which relies on the fact that, for mysterious reasons, $$\p_{\bar{z}} \frac{1}{z} = \delta(\bar{z}, z)$$. I would like to see complex analysis ~deleted~ rephrased in terms of delta functions anyway, so this is a good piece of doing it.
