...
One, the explanation for how to solve them that you get in undergraduate calculus is very philosophically unsatisfying. I don't like techniques that arising from "noticing" something and then proving it works. Of course that's how things are often discovered in the first place, but it's not a good long-term state to stay in. Noticing the trick might be difficult, and you might not be able to apply the same trick in other situations unless you get lucky enough to notice it again. It's much better if the insight somehow emerges from a natural, first-principles derivation that can be abstracted out and then reused or generalized.
