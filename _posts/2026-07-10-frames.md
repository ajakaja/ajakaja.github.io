...
The rest of the terms cancel because of $$\x_i \cdot \x^j = 1_{i=j}$$. Note how $$a_X$$ is a shorthand for $$\cdot \b{a}$$: lower indexes imply division, act like denominators, and are contravariant; upper indexes and numerators are covariant.
...
[^inner]: Aside: if we do not assume the Euclidean dot product, then it is also possible to define an inner product which makes $$(\x, \x + \y)$$ appear to be orthogonal, in which case $$a_1 (\x) + a_2 (\x + \y)$$ works the same way as $$a_x \x + a_y \y$$ did before. We are not considering this case. I think it is allways easier to think about there being two sets of vectors and one unchanging product, rather than one set of vectors but with the product changing (contrary to the usual approach in differential geometry...). Basically it is nice for $$\x^{-1} \cdot \x$$ to always equal $$1$$, since we are interpreting it as "measuring $$\x$$ in terms of $$\x$$" and what else would that be?
