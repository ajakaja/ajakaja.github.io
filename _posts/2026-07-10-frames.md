...
[^directsum]: The direct sum operation $$\oplus$$ acts more like a product than a sum (and, I understand, is equivalent for finite-dimensional spaces to the direct product $$X \times Y$$). It would be most accurate to explicitly write sense to write $$I_{x \oplus x}$$, but that's unwieldy. Another option is $$I_{x,y}$$, since in many cases commas are used to represent elements of direct sums, e.g. $$(v_x, v_y) \in \< x \> \oplus \< y \>$$, but $$I_{xy}$$ is unambiguous enough that I haven't seen a reason to switch.
...
Given a vector subspace $$X \sub V$$, there is an orthogonal subspace $$X_{\perp}$$ (which I pronounce "$$X$$-perp") such that $$V = X \oplus X_{\perp}$$. (The word 'orthogonal' here technically requires an inner product, but if one is not available you can call it the complementary subspace instead; the idea is the same.) The projection onto this subspace is

$$I_{\perp X} = I - I_X$$

In subscripts I always write $${\perp} X$$ instead of $$X_{\perp}$$, just because it is easier to read as the symbols become smaller.[^reject]

[^reject]: Sometimes this operator is called the 'rejection', but I don't realy like that name, because it's hard to use in a sentence: $$I_X$$ is the projection _onto_ $$X$$, so $$I_{\perp X}$$ is the... rejection... off of... $$X$$? There's not really a good way to say it. Calling $$I_{\perp X}$$ the 'rejector' for $$X$$ works, however.
...
5. The dual symbol $$\b{a}^*$$ is used whenever we write a denominator as a numerator instead, to remind us that it is 'acting like' a denominator. We mostly don't use it in its normal sense of dualization, but I am including it to connect to the way things are usually written.
...
Multiplying by $$\x$$ again therefore gives the [vector projection](https://en.wikipedia.org/wiki/Vector_projection):
...
In some fields you will see [dyadics](https://en.wikipedia.org/wiki/Dyadics) like $$\x\x$$ used to express the same idea as $$\x/\x$$, since for unit $$\x$$ they are equal: $$\x\x \cdot \b{a} = \x (\x \cdot \b{a}) = a_x \x$$. However, I think this notation is strictly inferior to my vector division. You really do want to represent the 'input' as a denominator, since for most purposes it acts like one. In the case of $$\x\x$$, the $$\x/\x$$ notation correctly handles the case where $$\x$$ is of non-unit length. It also generalizes in useful ways that dyadics cannot, to be described shortly. Juxtaposition notation _is_ useful for constructing tensors, especially of degree $$>2$$ like $$\x \y \z$$. It just doesn't work as well for linear transformations.
...
Generally speaking, dualization and conjugation operations across linear algebra have the behavior of inverting the _direction_ of vectors while preserving the _magnitude_, e.g. $$\overline{re^{i \theta}} = re^{-i\theta}$$ (which is well-defined for $$\bb{R}^{> 2}$$ by the way, $$\overline{re^{\vec{r} \cdot \vec{\theta}}} = re^{-\vec{r} \cdot \vec{\theta}}$$). This is fine... unless what you _actually_ want to be doing is measuring one vector in terms of another, like $$\x^{-1} (a_x \x + \b{a}_{\perp x}) = a_x$$, in which case, shouldn't you just actually be able to invert "all" of $$\x$$ to make the division work? This is basically the argument for vector division: it is the operation of measuring one vector in terms of another. And very rarely in geometry do you have to deal with the problem of thinking about vectors with norm zero (which are not really a problem anyway, if you allow for the generalized-inverse perspective, to be discussed later).[^measure]

[^measure]: Although I have defined $$\b{b} / \b{a} = \b{b} \o \b{a}^{-1}$$ here, it is often useful to actually cancel them out, letting $$\b{b} / \b{a} $$ mean $$\b{a}^{-1} \b{b}$$. There are arguments for both. I won't do that in this article just to avoid confusing things, but in other places I tend to use them interchangeably. The implied contraction is just a shorthand anyway. By the way: the basic reason this comes up for vectors but not scalars is that, in scalar algebra, $$b/a = a^{-1}(b)$$ and $$b/a = b \o a^{-1}$$ mean the same thing and act the same way in all calculations. Every term has one implicit index anyway, so there's no difference whether you trace over it or not. In vector algebra you have to actually distinguish the two cases. (In fact $$\b{a}^{-1} \cdot \b{b}$$ can be viewed as "really" being $$\b{b} \o \b{a}^{-1}$$ but implicitly viewed through its projection onto a scalar/spin-$$0$$ subrepresentation, so "measured" along the "basis" operator $$I_x = (\x \x^* + \y \y^* + \ldots)$$; this is a pretty good perspective actually but it's too far afield for this article.)
...
Which is the sense in which it is the 'inverse' of $$X$$. Equivalently, their matrix representations in any basis are pseudoinverses:
...
The rest of the terms cancel because of $$\x_i \cdot \x^j = 1_{i=j}$$. Note how $$a_X$$ is a shorthand for $$\frac{1}{X} \cdot \b{a}$$: lower indexes imply division, act like denominators, and are contravariant; upper indexes and numerators are covariant.
...
Frame division, therefore, involves contracting two frames on their frame index. We can write this out explicitly by labeling a basis frame for $$\cal{I}_X$$ as $$(e^1, e^2, \ldots)$$, where these vectors are not in bold because they are vectors in the index space $$\cal{I}_X$$, not the geometric space $$X$$. Then
...
So far we have discussed projections onto frames under the assumption that the vectors in the frames are orthogonal. This is the condition under which fractions expand termwise:
...
The complexity is very practical. When we start with a frame $$F = (\b{e}^1, \b{e}^2, \ldots) = (\b{e}^i)$$ and write a vector in terms of it

$$\b{a} = a_1 \b{e}^1 + a_2 \b{e}^2 + \ldots = a_i \b{e}^i$$

We want to be able to use the inverse frame $$1/F = (\b{e}_1, \b{e}_2, \ldots) = (\b{e}_i)$$ to extract these components:

$$\b{e}_i \cdot \b{a} = \b{e}_i \cdot (a_j \b{e}^j) = a_i$$

which requires that $$\b{e}_i \cdot \b{e}^j = 1_{i=j}$$. This is more restrictive than just the requirement that $$\b{e}_i \cdot \b{e}^i = 1$$, which is the stipulation filled by $$\b{e}^i / \| \b{e}^i \|^2$$; we _also_ require that $$\b{e}_i \cdot \b{e}^j = 0$$ for $$j \neq i$$. We will basically accomplish this by adding a term to $$\b{e}^i / \| \b{e}^i \|^2$$ which cancels out its dot product with all of the other $$\b{e}^i$$.

Consider a two dimensional frame as an example. Suppose we pick the non-orthogonal frame $$F = \b{f}^i = (\x, \x+\y)$$ to describe the vector space $$(\x, \y)$$. It is still possible to decompose vectors in terms of this basis, $$\b{a} = a_1 \x + a_2 (\x + \y)$$. However, the actual projections are no longer independent of each other: the value of $$a_1$$ and $$a_2$$ are _not_ given by $$a_x = (\x/\x) \b{a}$$ and $$a_y = (\y/\y) \b{a}$$. We can still write

$$\b{a}_F = I_{xy} \b{a} = \frac{F}{F} \b{a} = \frac{(\x, \x+\y)}{(\x, \x+\y)} \b{a}$$

But we _cannot_ factor it along each component, as

$$\frac{(\x, \x+\y)}{(\x, \x+\y)} \neq \frac{\x}{\x} + \frac{\x + \y}{\x + \y}$$

Instead, the correct projector has to use what is called the "dual" basis for the frame $$X$$. It turns out to be[^inner]

[^inner]: Aside: if we do not assume the Euclidean dot product, then it is also possible to define an inner product which makes $$(\x, \x + \y)$$ appear to be orthogonal, in which case $$a_1 (\x) + a_2 (\x + \y)$$ works the same way as $$a_x \x + a_y \y$$ did before. We are not considering this case. I think it is always easier to think about there being two sets of vectors and one unchanging product, rather than one set of vectors but with the product changing (contrary to the usual approach in differential geometry...). Basically it is nice for $$\x^{-1} \cdot \x$$ to always equal $$1$$, since we are interpreting it as "measuring $$\x$$ in terms of $$\x$$" and what else would that be?
...
So the factorization of $$F/F$$ happens componentwise by pairing the vectors of the numerator with the vectors of the denominator:

$$
\begin{aligned}
\b{a} &= \frac{F}{F} \b{a} \\
&= [\x \o (\x^* - \y^*) + \y \o \y^*] (a_1 \x + a_2 (\x + \y)) \\
&= a_1 \x + a_2 (\x + \y)
\end{aligned}
$$

Note that the dual frame is not an explicit denominator. Rather they are the dual vector components $$\b{e}_i$$ _of_ the denominator $$1/F$$. True, we could write them as explicit denominators as well,

$$
\begin{aligned}
\frac{(\x, \x+\y)}{(\x, \x+\y)} &\? \frac{\x}{(\x - \y)/2} + \frac{\x + \y}{\y} = \x \o (\x^* - \y^*) + (\x + \y) \o \y^*
\end{aligned}
$$

But there's little point to doing so. The inverses of the individual vectors like $$\frac{1}{2}(\x - \y)$$ are not really useful in this geometry at all. The only thing that it makes any sense to dualize is the entire frame at once, $$1/F = (\x, \x + \y)^{-1} = (\x^* - \y^*, \y^*)$$.

In general the inverse frame $$F^{-1}$$ must have matrix elements corresponding to the inverse of the matrix elements of $$F$$. The matrix representations are
...
(where I have explicitly written out the bases for each index space as a demonstration). The coordinate matrices are of course inverses:
...
This is supposed to be equivalent to the usual $$\adj(F)/\det(F)$$ construction in, um, untyped linear algebra. However I have so far been unable to a simple set of rules for doing doing this calculation purely symbolically. The problem is that it's not clear how to keep track of which index is left 'unbound' in the denominator (that produces the alternating minus signs when translated into the matrix inverse). It refers to a correct calculation on tensors of course, but I'm not satisfied with the shorthand. I'll update this if I come up with something.[^inverse]
...
In any case: if a denominator frame is not orthogonal, then we _cannot_ factor the frames apart term-by-term:

$$
\begin{aligned}
\frac{F}{E} = \frac{(\b{f}^1, \b{f}^2)}{(\b{e}^1, \b{e}^2)} \stackrel{!}{\neq} \frac{\b{f}^1}{\b{e}^1} + \frac{\b{f}^2}{\b{e}^2}
\end{aligned}
$$

Instead we have to factor them according to the dual frame $$E^{-1} = 1/(\b{e}^i) = (\b{e}_i)$$, always written with lower indexes when it is not in a denominator:

$$
\begin{aligned}
\frac{F}{E} = \frac{(\b{f}^1, \b{f}^2)}{(\b{e}^1, \b{e}^2)} = \b{f}^1 \b{e}_1 + \b{f}^2 \b{e}_2
\end{aligned}
$$

The values of the $$\b{e}_i$$ are functionally dependent on the _entire_ frame $$F$$, rather than just on $$\b{e}^i$$ alone.

Once we have defined $$\b{e}_i$$ in this way, everything we did with division continues to work. For example,

$$\frac{F}{E} \b{a} = \b{f}^i \b{e}_i \cdot (a_j \b{e}^j) = \b{f}^i (\b{e}_i \cdot a_j \b{e}^j) = \b{f}^i a_j 1_{i=j} = a_i \b{f}^i$$

We just can't use $$\b{e}_i = \b{e}^{i*}/\| \b{e}^i \|^2$$ anymore.
...
A linear transformation is invertible if both the numerator and denominator are nonsingular, which are the only frames we've been considering so far. The inverse is constructed by just swapping numerators and denominator frames, aka swapping upper and lower indexes:
...
$$\frac{AV}{V} = \frac{A(\x, \y, \z)}{(\x, \y, \z)} = \frac{(A\x, A\y, A\z)}{(\x, \y, \z)}$$

And its inverse can be written

$$A^{-1} = \frac{V}{AV} = \frac{A^{-1} V}{A^{-1} A V} = \frac{A^{-1} V}{V}$$

(it seems like it should be valid to multiply by $$A^{-1} / A^{-1}$$ here, but I'm not sure how to think about it)
...
[^av]: The expression $$\frac{1}{W} AV$$ seems to break the rule of denominators contracting with numerators, since $$AV$$ is two numerators in a row. This is unfortunate. The problem is that $$A$$ is "really" a pairing of a numerator and a denominator in the same term. It would be better if we always wrote linear transformations as ratios like $$A=F/E$$ to symbolize the fact that they have an input and output component.
...
With $$P = \frac{1}{E} F$$ as a change-of-basis matrix. (I would rather just leave it as $$E^{-1} F$$ but we should connect this to how it's usually written.) Note, we can write $$EP$$ or $$PE$$ for the result, because the index that connects them is implicit; I am writing $$EP$$ because it's more useful later.

In coordinates this is

$$\b{f}^i = (\b{e}^j \b{e}_j) \cdot \b{f}^i = (\b{e}_j \cdot \b{f}^i) \b{e}^j = P_j^{\, i} \b{e}^j$$
...
The transpose of an operator is literally the same operator but acting on a denominator on the left instead of a numerator on the right:

$$
\begin{aligned}
\frac{E}{F} \b{v} = A \b{v} \\ 
\b{u}^* \frac{E}{F} = A^T \b{u}^*
\end{aligned}
$$

Which is why transposes are sort of silly. Note that this is a lot like

$$\frac{1}{\b{v}} \frac{E}{F}  = (\frac{F}{E} \b{v})^{-1}$$

Except without using the actual inverse. As I touched on earlier, transposition/dualization/conjugation operations all consist of inverting the direction of an object but not its magnitude (e.g. $$re^{i \theta} \mapsto re^{-i \theta}$$). I believe the reason for this is that people are usually used to doing linear algebra, so they want linear maps, but they still want inverses at the same time, so inverting the direction is the best you can do. Mostly I suspect that this is a mistake and they should just fully invert objects including their scalars instead, but probably there are cases for both. In any case: the transpose is just what you get when want to treat $$A = E/F$$ as acting on a dual vector to the left instead of a vector to the right. I suppose it might be written 

$$A^T = \frac{F^{-1}}{E^{-1}}$$

to keep the notation consistent. In any case, it really refers to the same object $$A$$, just used in a different way.
...
where $$0^{\oplus n-k} = \underbrace{(0, 0, \ldots)}_{n-k\text{ times}}$$ is a 'singular' frame that describes the image of the kernel.
...
Here $$\bot$$ is a symbol that I'm using to represent a 'null' frame that matches nothing in a denominator---the 'empty frame', if you will. We have to have a different symbol than $$0$$ here, because it does not act the same as a zero frame in a denominator, as we will see in a second. You can think of $$\bot$$ as being an empty frame, $$\bot = ()$$, whereas $$0^{\oplus n-k} = (0, 0, \ldots)$$ is a frame of zeroes. It is raised to the $$(m-k)$$ power because that is how many dimensions it is annihilating (I'm also not sure if this is necessary, but it's nice for bookkeeping).[^bot]  

[^bot]: I can't really justify this notation; it just feels right. By the way, $$\bot$$ here is inspired not so much by the perpendicularity symbol, actually, but rather by 'bottom types' in type theory, which are types with no members (although it's convenient that it makes sense without this). I vaguely recall getting the notion from the [graphical linear algebra](https://graphicallinearalgebra.net/2015/12/14/26-keep-calm-and-divide-by-zero/) website, but I can no longer quite remember why, because they use the symbol differently than I am. Anyway, I can't see a way around using it. There really are two "kinds" of zero here. Which is curious.
...
I have written $$A_{\parallel} = F/E$$ for the 'non-trivial' part of $$A$$, the part that does something (corresponding to its nonzero singular values in its SVD, incidentally). Then I'm using $$\ker A$$ and $$\text{coker} A$$ as frames for their respective subspaces. We can also write $$A_{\parallel}$$ out as a ratio of frames as well:

$$A = \frac{\text{col}(A)}{\text{row}(A)} + \frac{0^{\oplus n-k}}{\ker A} + \frac{\text{coker} A}{\bot^{\oplus m-k}}$$

Note that $$\text{col}(A)$$ and $$\text{row}(A)$$ refer to frame which span the column and row spaces of $$A$$, but they don't correspond to the colums and rows exactly (which wouldn't work, anyway, because it would be double-counting all the coordinates), and there are many ways to pick these frames that give the same transformation.
...
Incidentally, the SVD tells us that it is always possible to pick the numerators and denominators in $$A = F/E = (\b{f}^1, \b{f}^2, \ldots)/(\b{e}^1, \b{e}^2, \ldots)$$ to be _simultaneously_ orthogonal. I haven't yet thought of a way to see intuitively that this must be true.

------

Now consider trying to invert $$A$$. Only the non-trivial part $$A_{\parallel}$$ is invertible. Inverting only this part, and setting the rest to zero, gives the [Moore-Penrose Pseuodinverse](https://en.wikipedia.org/wiki/Moore%E2%80%93Penrose_inverse) of $$A$$, which I have also written about [here]({% post_url 2023-09-25-inverses %}) (I don't care for the $$A^+$$ notation so I'm only going to use it this once):
...
The kernel and cokernel have swapped roles. Now $$\text{coker}(A) \subset W$$ is the set of vectors which are mapped to $$0_V$$, since they have no preimage in $$V$$, while $$\text{ker}(A) \subset V$$ is the cokernel of $$A^{-1}$$, consisting of the vectors that were all mapped to $$0 \in W$$ by $$A$$. However, the cokernel and kernel still act differently. The kernel vectors are _all_ in the preimage, so they are all accessible. We represent this by dividing by zero. 

But now we have a different interpretation: instead of saying that these vectors are inaccessible from $$A^{-1}$$, we can think of them as _all_ being accessible. The correct preimage $$A^{-1}(0_W)$$, in a generalized inverse sense, is that it is _all_ of the vectors in the kernel of $$A$$. The $$1/0^{\oplus n-k}$$ tells us that we would need $$(n-k)$$ additional pieces of data to describe the precise vector in the preimage, similar to inverting $$0 \times a = 0$$ to get $$0^{-1}(0) \? a$$ requires recovering the missing information of the value $$a$$.

The implication is that the preimage of any point $$\in A^{-1}(W)$$ includes the entire subspace $$\text{ker}(A)$$ at once, which _would_ take $$(n-k)$$ additional parameters to describe an individual point in, if we had them. We can write this as a linear transformation from an additional $$(n-k)$$ pieces of data $$\Lambda^{n-k} = (\lambda_1, \lambda_2, \ldots \lambda_{n-k})$$ into the kernel:

$$\frac{\text{ker} (A)}{0^{\oplus n-k}} \? \frac{\text{ker} (A)}{\Lambda^{n-k}}$$

So we think of this "unbound" linear transformation as a description of the entire kernel at once. (Note, we could also have written $$0/\ker A$$ and forgotten the $$(n-k)$$ information, and that would probably characterize $$A$$ more correctly. But I like to keep it around because it elucidates the structure better.)
...
Which is "correct" in the sense that it's the best you can do to invert $$A$$. $$A$$ maps $$\x \lra \b{a}$$ so those get swapped, and it maps $$\y \ra 0$$ so the preimage contains a the whole one-dimensional subspace $$\< \y \>$$, and it maps nothing to $$\b{b}$$ so the preimage does not produce a factor of $$\b{b}$$ at all. Note that the separate use of $$0$$ and $$\bot$$ allows us to express some slightly different objects pretty succinctly as well. For example, consider $$A \mid_{\< \x \>}$$, the _restriction_ of $$A$$ to $$\< \x \>$$. Now we have no action on $$\y$$ at all:

$$A \mid_{\< \x \>} = \frac{\b{a}}{\x} + \frac{\b{b}}{\bot}$$

We could also wonder about the case where we really do map $$\y \ra \bot$$ instead of $$0$$, so we "erase" the component rather than zeroing it. This strays away from the usual formulation of linear algebra, but why not?

$$A' = \frac{\b{a}}{\x} + \frac{\bot}{\y} + \frac{\b{b}}{\bot}$$

Etc. It is interesting to think about.

Another case to explore is to let the denominator frame vary with a parameter in order to see how the formulas 'switch over' when it becomes singular. Write
...
Leaving the $$\lambda$$ unspecified, however, produces the entire preimage of that point, which is the best $$A^{-1}$$ can be asked to do.
...
Frames work the same way on differentials, if you regard $$(dx, dy)$$ and $$(\p_x, \p_y)$$ as dual frames to each other. I think the best way to do this is to pick one symbol and use it for both. In this section I will use $$dx$$ for differentials and $$1/dx$$ for their pseudoinverses, the partial derivatives (_not_ total derivatives). So differentials correspond to $$dx^i$$ and partial derivatives to $$dx_i$$.[^deriv]
...
Admittedly $$f_{x,y} = (f_x, f_y)$$ is pretty awkward. Ideally we would write $$f_{xy}$$ in analogy with the vector version $$\b{a}_{xy}$$ earlier, but unfortunately that notation is already taken by the second partial derivative, so $$f_{x,y}$$ will have to do. All of this is a bit of a mess because the shorthand expression for derivatives should really be $$df_{dx}$$ (after all $$\p_x f$$ is the $$dx$$ component of $$df$$!), but even I can't bring myself to do that yet.
...
&= (\p_i a_j) du^i du^j + a_j (\Gamma_{ik}^j du^k) du^i
\end{aligned}
$$

Where we've relabeled the indexes and used the Christoffel symbol $$\Gamma_{ik}^j du^k = (\p_i du^j) du^k$$ for the derivative of the frame with respect to itself. My version is much easier to read!

One can similarly take the derivative of a tangent vector field $$d_U (\b{a}_U) = d_U (a^U \frac{1}{U})$$, or any higher-degree tensor. Although my presentation here is a bit muddled because of jumping between $$d\b{u}$$ and $$U$$, the reason I mention it is that I find the whole expression so much easier to expand without indices, in terms of frames and the pseudoinverses which project everything onto the surface again. In particular, it allows you to do calculus with vector fields without having to immediately write them out in a basis of derivatives or differentials. If you always tihnk of your vector fields $$a_U = (a_1, a_2, \ldots)$$ as "really" being written in frames $$a_U U$$, then it is perfectly natural that $$d (a_U U)$$ has a product rule term $$(d a_U) U + a_U (d U)$$.
