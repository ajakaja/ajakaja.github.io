...
Here's an example. When we start with a frame $$F = (\b{e}^1, \b{e}^2)$$ and write a vector in terms of it

$$\b{a} = a_1 \b{e}^1 + a_2 \b{e}^2$$

We want to be able to use the inverse frame $$1/F = (\b{e}_1, \b{e}_2)$$ to extract these components:

$$\b{e}_i \cdot \b{a} = \b{e}_i \cdot (a_j \b{e}^j) = a_i$$

Meaning that $$\b{e}_i \cdot \b{e}^j = 1_{i=j}$$.

Now suppose we pick a non-orthogonal frame $$F = \b{f}^i = (\x, \x+\y)$$ to describe the vector space $$(\x, \y)$$. It is still possible to decompose vectors in terms of this basis, $$\b{a} = a_1 \x + a_2 (\x + \y)$$. However, the actual projections are no longer independent of each other: the value of $$a_1$$ is _not_ given by $$a_x = (\x/\x) \b{a}$$. We can still write
...
Instead, the correct projector has to the use the "dual" basis for the frame $$X$$ in the denominator. It turns out to be
...
We _could_ write these vectors out as denominators as well,
...
But there's little point. The inverses of the individual vectors like $$\frac{1}{2}(\x - \y)$$ are not really useful in this geometry at all. The only thing that it makes any sense to dualize is the entire frame at once, $$1/F = (\x, \x + \y)^{-1} = (\x^* - \y^*, \y^*)$$.[^inner]

[^inner]: Note: if we do not assume the Euclidean dot product, then it is possible to define an inner product which makes $$(\x, \x + \y)$$ appear to be orthogonal, in which case $$a_1 (\x) + a_2 (\x + \y)$$ works the same way as $$a_x \x + a_y \y$$ did before. We are not considering this case. I think it is alkways easier to think about there being two sets of vectors and one unvarying product, rather than one set of vectors but with the product changing (contrary to the usual approach in differential geometry...). It is nice for $$\x^{-1} \cdot \x$$ to always equal $$1$$, since we are interpreting it as measuring $$\x$$ in terms of $$\x$$" and what else would it be?
...
(where  for clarity I have explicitly written out the bases for each index space.) The coordinate matrices are of course inverses:
...
The takeaway is that, if a denominator frame is not orthogonal, then we _cannot_ factor the frames apart term-by-term:
...
(...multiplying by $$A^{-1} / A^{-1}$$ seems like it should be valid here...)
