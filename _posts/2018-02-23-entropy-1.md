...
I have been reviewing concepts from [Information Theory](https://en.wikipedia.org/wiki/Information_theory) this week, and I've realized that I never quite understood what (Shannon) [Entropy](https://en.wikipedia.org/wiki/Entropy_(information_theory)) was all about.
...
We imagine receiving an stream of events from $$P$$: "something happened", and breaking it down into *which* event happened.  The values from $$P$$ can only take on a single value, with $$\log 1 = 0$$ bits of entropy -- meaning that, as the number of events goes to infinity, the data required per bit goes to 0, because $$\lim_{N \ra \infty} \frac{\log N}{N} = 0$$. The stream of values from $$i$$ would naively be representable in $$\log \vert i \vert$$ information, but, knowing the probabilities lets us compress them into $$H[p] = - \sum p_{i} \log p_{i}$$ information.

We can also hypothetically imagine further decomposing each of the $$i$$ events into $$m_{i}$$ microstates, which take $$\log m_{i}$$ data to distinguish between. If the probabilities are irrational numbers, maybe we just do this to some reasonable fidelity. Or maybe it's just a tool for understanding. It doesn't really matter.
