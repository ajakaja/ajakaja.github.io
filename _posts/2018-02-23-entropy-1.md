...
This is better thought of as the 'expected value of $$-\log p(x)$$':

$$H[p] = \bb{E}[- \log p(x)]$$

I claim that $$- \log p(x)$$ somehow captures the amount of information that we learn if we get the result $$x$$, and so entropy somehow measures the expected value of information when sampling from this distribution.

Note that this equation for entropy is *always positive*. The negative sign is misleading: since $$p(x)$$ is always less than 1, $$\log p(x) < 0$$. It would probably be better to always write $$\log \frac{1}{p(x)}$$ to emphasize this, but that's not usually done. Also, since $$0 \log 0 = 0$$, because $$\lim_{p\ra 0^{+}} p \log p = \lim_{N \ra \infty} \frac{-\log N}{N} = 0$$, 0-probability events never happen and contribute nothing to entropy.
...
This means, intuitively, that as we make a sequence of $$N \ra \infty$$ outcomes from this unfair coin (with $$\ra \frac{3}{4}N$$ `H`s), it will be possible to compress that sequence into a sequence of $$\ra 1.642N$$ bits, since that is another way to communicate the same volume of information.
