...
In the previous post, I looked at how some operations related to exterior algebra (wedge, 'meet', interior product, Hodge Star, geometric product) correspond roughly to set operations (union, intersection, subtraction, complement, symmetric difference), but the analogy wasn't very good. Exterior algebra isn't _exactly_ linearized set algebra. So what is it?

I figured out the answer: it's linearized [oriented projective geometry](https://en.wikipedia.org/wiki/Oriented_projective_geometry) (henceforth 'OPG'). You could also say this the opposite way: OPG is exterior algebra if you forget about all the scalars. Either way they're related. OPG, incidentally, holds the mantle of "the obscure math topics I think is most likely to be mainstream someday" -- it just needs a really good application to get there.

I read through (most of) "Oriented Projective Geometry" by Jorge Stolfi (1991), which is a great book that I highly recommend, though it could really use more concrete examples. Stolfi knew about the relationship to exterior algebra also, and he cited the same Barnabei/Brini/Rota paper I've been looking at. He also had some better notations and a few ideas which I should have been aware of. It will take way too much space to explain projective geometry here, or to do justice to the oriented version, but I will mention the key ideas needed for the comparison to exterior algebra.
...
An example to show why you would want to do this: recall that we can define lines in $$\bb{R}^2$$ by an equation like $$(a,b) \cdot \b{x} = c$$. Homogenous coordinates basically switch to referring to lines by $$(a,b,-c) \cdot (\b{x}, 1)$$, adding a new coordinate which is always 1 -- so we can refer to any line in $$\bb{R}^2$$ (the set of values $$\b{x}$$ which satisfy the constraint) by providing a single vector $$(a,b,-c)$$, or any positive scalar multiple of it, such as $$(a/c, b/c, -1)$$ (if $$c > 0$$). This is nice because we happen to be pretty good at computing with vectors already.

A point in $$\bb{O}^n$$ is defined as equivalence class of vectors in $$\bb{R}^{n+1}$$, where two vectors are equivalent if they differ by positive scalar factor[^rp]. We use _positive_ scalar multiples because, for instance, $$(a,b,c)$$ and $$(-a,-b,-c)$$ refer to different _directions_ in $$\bb{R}^{3}$$, and thus (by definition) to different points in $$\bb{O}^2$$.
...
Clearly the relevant information to define a point in $$\bb{O}^2$$ is two real numbers $$(x,y)$$ and a 'sign flag', which can be $$\{+1, 0, -1\}$$. I like to write the sign flag as a $$w$$-coordinate, in the first position, so that it is the first index in any dimension.

$$(\sgn, x,y) \in \bb{O}^2$$

The idea behind not allowing division by negative numbers is that points, lines, etc maintain _orientations_. It's not hard to imagine why we might want to keep track of the orientation of a line. Points are a little trickier; for a simple argument why you might care about oriented points, consider that a 1d slice of an oriented line seems to be a point that goes 'into' or 'out of' the page, depending on the direction of the line, and this information might be needed if, say, you want a calculation in 2d to hold if you take a 1d slice of the whole system.. But there will be better motivations later. It's easiest to just not divide through, when doing calculations, to avoid having to be careful about signs and absolute values.
...
It is easy to visualize this embedding for, say, $$\bb{O}^1$$: consider the real plane $$\bb{R}^2$$ with vertical lines drawn at $$x=1$$ and $$x=-1$$, which will be the "front" and "back" parts of $$\bb{O}^1$$. Points in $$\bb{O}^1$$ correspond to "directions" in $$\bb{R}^2$$, which are determined by taking vectors $$(x,y)$$ and seeing where they intersect one of the two vertical lines. If $$y > 0$$, the intersection point is $$(1, x/y)$$, on the front line; if $$y < 0$$, the point is $$(-1, x/y)$$ on the "back line". Like this:
...
One learns to think with this model when looking at geometric objects. We see every point as unique 'direction' in some enclosing space, and two distinct points are _always_ non-redundant (linearly independent), but three collinear points, or four coplanar ones, etc are redundant (linearly dependent). It's easy to see why this view works better for geometry, since the origin really _isn't_ a privileged point if we just want to talk about shapes and their relationships, and it shouldn't matter if two points are in the same line from the origin or not, when considering their geometric properties.
...
An oriented line in $$\bb{O}^n$$ is described by any two points $$a,b$$ on it (in the correct order), via the simplex $$(ab)$$. The value of the simplex as a multivector is:
...
$$\det \begin{pmatrix} 1 & a_x & a_y \\ 1 & b_x & b_y \\ 1 & c_x & c_y \end{pmatrix} = 0^{\v 2}$$
...
We can convert the constraint that $$(1, \vec{c}) \v (ab) = 0$$ into a typical equation for a line in $$\bb{R}^2$$ like so:
...
$$\vec{c} \v (\vec{b} - \vec{a})  = \vec{a} \v \vec{b} \tag{line in plane constraint}$$
