...
So while I'm all for the "let's teach everybody to code!" movement, I wish we'd stop writing yet another, like, Learn Machine Learning With Python Tutorial, or whatever, and just make maybe take some time to work on making everything the world around us better in little incremental ways, by making what we've already got _suck_ less, for ourselves and for all the newcomers and for just eveyone, so we can have less stress and more peace in our lives. 

Because: some days I can't honestly tell anyone they should get into this, when on a good day you get to slowly hack your way through bullshit and on a bad day you might just succumb and give up.
...
Hrm. Okay, I guess I regret clicking "open in new tab", so I'll just go back and open it the right way:

{% include image.html filename="2018-04-19/5-click.png" width="400px" caption="" %}

Phew, it actually downloads. I wonder why it worked that time.
...
And... oh. It opened in Xcode. I didn't want that. Why would I want that? Why would Apple default to that? I never use Xcode. Tons of people never use Xcode.

Well, there's no way to say what app to open it with from the chrome browser. I looked, for a while:

{% include image.html filename="2018-04-19/8-chromesettings.png" width="600px" caption="" %}

Weird, I really thought that existed years ago in Chrome. Why would they take something so useful out?
...
The first result is from some site I did not want Chron.com, and turns out to be under "Small Business» Types of Businesses to Start» Open a Bar»" on their navbar, and is for Windows only, and isn't even what I want, and is probably years old (there's no date), and is a horribly manual way to do anything through the OS instead of just telling the browser what you want:

{% include image.html filename="2018-04-19/10-chron.png" width="400px" caption="" %}

I just wanted to see the _right_ way to do it, which should just be documented on the browser website, though it's not like I've come to trust Google to do anything human-centric. So I guess I'll ask a bit more specific question because, fortunately, these struggles are so common that people more sociable than I go ask about them:
...
The second is on 'superuser.com', which is apparently a StackExchange for computer.... superusage... which bodes well, because most of the community references on the internet are total trash and StackExchange/Overflow/derivatives are the only thing keeping any of us sane. 
...
So it's probably not happening. Sigh.

Now, I can work around this by reading the file locally, but I'm still annoyed at this point. Why do things have to be bad, on purpose? Why can't it just be good?
...
Oh. Er. No. No, um... I wanted to open it, and I don't care that it's from an unidentified developer. Please? ~~Infuriating~~ Awkward.
...
The first article wants to show notifications (why? why does anyone do this?), has two sets of social media buttons (why? does anyone click those? please stop doing it so they go away. And why _two_?) and a bunch of other screen-filling garbage, and is from 20_16_ (maybe I should have searched for `High Sierra`)...

{% include image.html filename="2018-04-19/32-tekrevue.png" width="400px" caption="" %}

...but at least does recognize the issue and tells us that, yes, we're not imagining things; Mac has been making this system more irritating with each release, and there is a way to fix it:

```
sudo spctl -–master-disable
```

But before we `sudo` a random command, maybe let's check what it does? 
...
Well, it's not vey helpful. Maybe I don't want to 'master-disable', we just want to disable _one_ policy? But when your APIs are stringly-typed and your documentation doesn't document them... Ugh, I don't know; this is not what I wanted to do today. I just wanted to see how to make a line plot. I give up. Let's just do it:
