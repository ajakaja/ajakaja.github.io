...
In [Factorials as Multiplicative Integrals]({% post_url 2025-05-02-factorials %}) we discussed how the double-factorial's asymptotic behavior is different between even and off factorials, and the discrepancy is entirely due to the fact that, when written as an multiplicative integral, it is over a slightly different integration range:
...
S_{n-1}(1) &= \frac{\pi^{n/2}}{\frac{1}{2}\Gamma(\frac{n}{2})}\\
\end{aligned}
$$
...
Probably no; that seems like too strong a statement. That association is not strong enough to regard as "causality". We need something to connect with the combinatoric interpretation of factorials: when computing $$S_{2k-1} = 2 \pi^k/(k-1)!$$, why does the denominator involve $$(k-1)!$$ at all? What does it have to do with permutations and factorials in general?
...
Fractional Calculus is what you get when you try to generalize the derivative formulas of calculus 
...
$$
\begin{aligned}
D^{1/2} x &\? \frac{x^{1/2}}{(\frac{1}{2})!} = \frac{\sqrt{x}}{\sqrt{\pi} / x} \\
D^{1/2} 1 &\? x^{-1/2}{(\frac{-1}{2})!} = \frac{1}{\sqrt{\pi x}}
\end{aligned}
$$

(The latter is disputable, because it's not going to gives $$(D^{1/2})^2 (1) = D(1) = 0$$. Some kind of tricks, involving distributions no doubt, are needed to get it right.)

Or (using the second), should this be multivalued across a bunch of roots of unity or something?

$$D^{1/k} e^{ax} = \sqrt[k]{a} e^{ax}$$

Another thing that defies simple explanation is that a fractional derivative is inherently _nonlocal_. Whereas a discrete derivative only involves points in the immediate vicinity of where you cmpute it:

$$D_x f(x) = \lim_{dx \ra 0} \frac{f(x + dx) - f(x)}{dx}$$

The fractional derivative ends up equivalent to an integral over the whole domain of the function. The simplest way of seeing this is to consider a discretized version of derivative:
...
Where $$T_{dx}^k f(x) = T_{k \d x } = f(x + k \d x)$$. For integer $$k$$ every term is near where you started, and so in the limit $$dx \ra 0$$ they become an infinitesimal measure of the change in $$f(x)$$. But one fractional derivative comes by just replacing $$k$$ with a non-integer here, and then using the (!!) non-integer binomial expansion, which has _infinitely_ many terms:

$$D^n_x = \frac{1}{dx^n} ( \sum_{k = 0}^{\infty} \binom{n}{k} T_{dx}^k (-I)^{n-k})$$

Which is actually very hard to interpret from the start, since $$(-1)^{n-k}$$ is a non-integer power and so is normally thought of as multivalued (being equal to any of the $$k$$'th roots of unity $$\omega_k^j = \{ e^{2 \pi i j/k}\}$$), and what is that supposed to mean? So, okay, maybe you generalize from the "backwards" derivative
...
$$
\begin{aligned}
D^n_x &= \frac{1}{dx^n} ( \sum_{k = 0}^{\infty} \binom{n}{k} T_{-dx}^{n-k} (-I)^{k}) \\
&=\frac{1}{dx^n} (1 - \binom{n}{1} T_{-dx} + \binom{n}{2} T_{-dx}^2 + \binom{n}{3} T_{-dx}^3 - \ldots)
\end{aligned}
$$

and then you have one fewer problems but still many, because what is the meaning of the binomial expansion for non-integer values anyway? Well, people say that lots of combinatorics continues to work when you let the factorials become Gamma functions:

$$\binom{1/2}{1} = \frac{(\frac{1}{2})!}{(1)! (\frac{1}{2}-1)!} = \frac{\frac{\sqrt{\pi}}{2}}{\sqrt{\pi}} = \frac{1}{2}$$

Because mysterious fractional binominal expansions end up giving perfectly-legitimate _Taylor series_, such as

$$(1 + x)^{1/2} = 1 - \frac{1}{2} x + \frac{1}{4} x^2 - \frac{3}{8} x^3 + \ldots$$

Which converges if $$\| x \| < 1$$. Disconcertingly, you can also expand the sum into negative powers instead of positive ones (so the terms range over $$\sum_{k=-\infty}^{-1}$$), which gives a Taylor series for the same function but around $$x=\infty$$ instead of $$x=0$$. (And generally the generalized binomial coefficients on non-positive-integers obey [almost all](https://en.wikipedia.org/wiki/Binomial_coefficient#Generalization_to_negative_integers_n) of the same algebraic identities as the usual ones, so there's definitely _something_ going on there.)
...
&= \frac{1}{\sqrt{dx}} (I - \frac{1}{2} T_{-dx} + \frac{1}{4} T_{-2 dx} -  \frac{3}{8} T_{-3 dx}\ldots) \\
\end{aligned}
$$

Which basically amounts to an _integral_: if you imagine taking the limit $$dx \ra 0$$ while extending the sum to $$N \ra \infty$$ terms such that $$N \d x = a$$ is fixed, then this is something like

$$D^{1/2}_x f(x_0) \sim \int_{x_0}^{x_0-a} f(x) \underbrace{\sqrt{dx}}_{\text{??}}$$

Whatever that means (not correct, though; I don't know how to include the binomial factors in the integrand). Point is, it's a _non_-local calculation, which means that "whatever it is", it's not acting all that much like a derivative anymore, is it? Also, not only is it an integral, but we seem to have some freedom over _which_ integral it is depending how we take the limits? This is the source of _some_ of the variation in definitions of fractional derivatives (see [here](https://mathoverflow.net/questions/285186/why-are-there-so-many-fractional-derivatives)) but not all of it. (Incidentally, this is the same problem you get when trying to define an integral as an "inverse derivative operator": clearly $$D_x^{-1} f(x) \? \int f(x) \d x$$ needs to get its integration bounds from somewhere, also.)

Fractional calculus is evidently very closely connected to "fractional combinatorics", which means that to assign meaning to it we really do need to be able to say what's going on with $$(1 + x)^{1/2}$$ as well. And a satisfying answer for _that_ is necessarily going to involve contending with interpolating the permutations of fractional sets. I have no doubt that that's where we're going, as it seems like the heart of the thing... but it's too early for that. This article is just about how staring at the walls from different directions and looking for cracks.

Some people have tried to find geometric interpretations for fractional derivatives (see e.g. [here](https://math.stackexchange.com/questions/477888/what-does-a-half-derivative-mean) here), but I think most would agree that the best explanations available are still quite sketchy. I skimmed some books and found very little in the way of interpretation. Still, they are quite well-_behaved_, as numerous examples show: the fractional derivatives and integrals do, for the post part, cleanly interpolate between their integer equivalents.

The best reference I have found on this is a [pdf](https://www.reed.edu/physics/faculty/wheeler/documents/Miscellaneous%20Math/Fractional%20Calculus/A.%20Fractional%20Calculus.pdf) "Construction & applications of the fractional calculus" by Nicholas Wheeler (whose lucid writing on weird math I have also plugged elsewhere...). But still Wheeler does not seem to find a concrete physical interpretation; it just works, giving the right answer on real physical problems.

However. Wheeler does come right up to (before sadly turning around) what I think is the most important relationship I've seen so far:

I mentioned earlier that the volume of the $$n = 2k$$-sphere is compellingly written in a formulate that looks like the volume of an $$k$$-simplex with side-length $$\pi r^2$$:

$$V_{2k}(r) = \frac{(\pi r^2)^k}{k!} = \frac{a^k}{k!}$$

And this formula does work for half-integer $$k$$. Even more interesting, though, is how the half-derivative with respect to $$a$$ takes you from $$V_{2k}$$ to $$V_{2k-1}$$:
...
The source of the $$\sqrt{\pi}$$ is in there somewhere. The interpolated derivative is treating the one coordinate $$a$$ as an area in _two_ coordinates $$\pi r^2$$ such that you can remove "half" a coordinate from it fully-symmetric way. And for some reason, I guess, this requires including that factor of $$\pi$$?

It's weird. We always had the elementary option of computing $$\p_{\sqrt{x}}$$ by rewriting $$x = y^2$$:

$$\p_{\sqrt{x}} \frac{x^k}{k!} = \p y \frac{y^{2k}}{k!} = 2 y^{2k-1}{(k-1)!} = 2 \frac{x^{k-1/2}}{(k-1)!}$$

That is, without the $$\pi$$ factor. But iterating this doesn't cancel out correctly at all

$$
\begin{aligned}
\p_{\sqrt{x}}^2 \frac{x^k}{k!} &= (2k-1) \frac{x^{k-1}}{(k-1)!} \\
\p_{\sqrt{x}}^{2k} \frac{x^k}{k!} &= (2k-1)!!
\end{aligned}
$$

And the $$\sqrt{\pi}$$ factor basically serves to fix this: $$\p_{\sqrt{x}}$$ is slightly too "large", by a factor of $$\sqrt{\pi}$$, to reduce $$x^k/k!$$ down to $$1$$. Whereas writing $$a = \pi r^2$$ produces a parameterization in two variables $$r = a/\sqrt{\pi}$$ that together are somehow the same "size" as $$a$$.

In light of this it seems like not-a-coincidence that $$\int_{\bb{R}} e^{-x^2} \d x  = \sqrt{\pi}$$, because it's really $$\int_{\bb{R}} e^{-\pi r^2} \d x$$ that's equal to $$1$$. You can't even stick a quadratic in an exponential without contending with this, whatever it is.

But beyond that, I dunno. Interesting. Seems suggestive. Something to think about. Pretty neat I guess. I'm lost.
