...
$$\p_{\sqrt{x}} \frac{x^k}{k!} = \p_y \frac{y^{2k}}{k!} = 2 \frac{y^{2k-1}}{(k-1)!} = 2 \frac{x^{k-1/2}}{(k-1)!}$$

That is, without the $$\pi$$ factor. But iterating this doesn't cancel out in the correct way to make a fractional derivative at all:
...
The $$\sqrt{\pi}$$ factor basically serves to fix this: $$\p_{\sqrt{x}}$$ is slightly too "large", by a factor of $$\sqrt{\pi}$$, to reduce $$x^k/k!$$ down to $$1$$. Whereas writing $$a = \pi r^2$$ produces a parameterization in two variables $$r = a/\sqrt{\pi}$$ that together are somehow the same "size" as $$a$$.

In light of this it seems like not-a-coincidence that $$\int_{\bb{R}} e^{-x^2} \d x  = \sqrt{\pi}$$, because it's really $$\int_{\bb{R}} e^{-\pi r^2} \d r$$ that's equal to $$1$$. You can't even stick a quadratic in an exponential without contending with this, whatever it is.
