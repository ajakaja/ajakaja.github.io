...
*In this article $$\v$$ denotes the 'join' of simplexes and the exterior product, and $$\v$$ is the 'meet' of simplexes, and the dual of $$\v$$ with respect to $$\star$$.*
...
$$\text{vol}(a,b,c) = \frac{1}{2}(\vec{b}-\vec{a}) \v (\vec{c}-\vec{a})$$
...
$$\text{vol}(a,b,c) = \frac{1}{2}(\vec{a} \v \vec{b} + \vec{b} \v \vec{c} + \vec{c} \v \vec{a})$$
...
$$\text{vol}(a,b,c) \stackrel{?}{=} \frac{1}{3!} \begin{pmatrix} 1 & a_x & a_y \\ 1 & b_x & b_y \\ 1 & c_x & c_y \end{pmatrix}$$
...
The $$\b{w} \cdot$$ prevents us from including the $$\vec{a} \v \vec{b} \v \vec{c}$$ term, which was zero in $$2d$$ but which is the volume of the tetrahedron $$(\mathcal{O}abc)$$. It's a meaningful quantity, but not for computing the area. 
...
This appears to be the same result as $$\b{w} \v (\vec{a} \v \vec{b} + \vec{b} \v \vec{c} + \vec{c} \v \vec{a})$$, but they are subtly different. A $$\v^2$$ power of a matrix creates another matrix, not a multivector. Presumably our matrices are linear transformations between some vector spaces $$U \ra V$$, where $$U$$ has some basis $$\{ \b{e}_i \}$$, and the result of taking the second wedge power would actually be a linear mapping $$\v^2 U \ra \v^2 V$$:
...
A more interesting plausible answer: the matrix of a simplex can be viewed as a linear transformation from some abstract 'simplex space', which I'll call $$S$$, to the space we are interested in. A single component $$\b{e}_i$$ refers to a point, and we have an infinite 'supply' of points, in that this space can have as many dimensions as we want; call this the space $$S$$. A bivector $$\b{e}_i \v \b{e}_j \in \v^2 S$$ refers to lines, and we have an infinite supply of lines also; etc for higher dimensions. We build simplexes as linear transformations from our infinite supply of objects $$\v S$$, which have no concept of geometry at all and where every point and line is distinct, into whatever concrete space we're talking about. This amounts to adding structure: a coordinate system, an origin, and relative positions between the objects we previously only considered abstractly.
...
(Since we prefer right-multiplication, this suggests that the simplex matrix should actually be represented by its transpose, but I don't want to rewrite everything I've done so far so we'll just deal with it.)
...
Of course, we can only compute $$\p$$ if we get the figure as a list of points, or as a multivector in homogenous coordaintes. The vector value $$\vec{a} \v \vec{b} \v \vec{c}$$ would not help us at all with computing $$ab + bc + ca$$. Nevertheless, this all makes me wonder if perhaps the $$\b{w}$$-coordinate in OPG is not necessary and could be removed if we were at taking wedge products. More on that next time...
...
## 5. Scalar Volumes and Cayley-Menger Determinants
...
Some brief manipulations turn this into [Heron's Formula](https://en.wikipedia.org/wiki/Cayley%E2%80%93Menger_determinant) using the semiperimeter $$s = \frac{A + B + C}{2}$$: $$A^2 = s (s-A) (s-B) (s-C)$$. I've long wondered where that comes from, and I guess I still don't really quite get it but at least it's clear where to start if I wanted to generalize to other figures.
