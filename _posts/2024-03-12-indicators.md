...
$$
\begin{aligned}
\int 1_{(a,b)} f' \d x  &= \int (\theta_a - \theta_b) f' \d x \\
&= \int_a^b f' \d x \\
&= f(b) - f(a)
\end{aligned}
$$

We can also use integration by parts to move the derivative onto the indicator function. The $$f' = \p_x f$$ passes its derivative over to $$(-\p_x) 1_{(a,b)}$$, which ends up acting like the boundary operator on the underlying oriented surface $$(a,b)$$:[^boundary]
...
$$
\begin{aligned}
\int 1_{(a,b)} f' \d x &= \int 1_{(a,b)} [\p f] \d x \\
&= \int [(-\p_x ) 1_{(a,b)}] f \d x \\
&=\int [(-\p_x) (\theta_a - \theta_b)] f \d x \\
&= \int (\delta_b - \delta_a) f \d x \\
&= f(b) - f(a) \\
\end{aligned}
$$

$$(-\p_x ) 1_{(a,b)}$$ looks kind of like an indicator function for the boundary of $$(a,b)$$, that is, for the surface $$\p(a,b) = (b) - (a)$$. But it can't be, because indicator functions necessarily have value $$1$$ at points in their predicate, while $$(-\p)1_{(a,b)} = \delta_b - \delta_a$$ has value $$\infty$$, ish (delta functions do not really "have" values at $$x=0$$ but you know what I mean). 

In fact we can integrate against the indicator function $$1_{\p(a,b)} = 1_{(b) - (a)} = 1_{b} - 1_{a}$$. It's just that it belongs in a "zero-dimensional integral" instead of a "one-dimensional integral". That is, there's no $$dx$$:

$$
\begin{aligned}
\int [1_{b} - 1_{a}] f &= \int_b f - \int_a f \\
&= \int_b f - \int_a f \\
&= f(b) - f(a)
\end{aligned}$$

This is a notation that you see around occasionally. Basically it treats "evaluation at a point" as a type of integral as well. In the same way that one-forms like $$f(x) \d x$$ are things that get evaluated on the vectors that comprise curves ($$1$$-surfaces) by integrals, zero-forms like $$f(x)$$ are things that get evaluated on points ($$0$$-surfaces), also by integrals. In particular this makes the notation of the [Generalized Stokes' Theorem](https://en.wikipedia.org/wiki/Generalized_Stokes_theorem) applicable to $$0$$-forms as well: $$\int_{\omega} \d f = \int_{\p \omega} f$$. If $$\omega$$ is an interval and $$\p \omega$$ is a pair of oriented points $$(b) - (a)$$, you need to allow for $$\int_{\p \omega} f = f(b) - f(a)$$ in order for the same notation to apply.
...
The resulting sign $$\sgn(I)$$ is determined by whether the range of integration $$I$$ was over a positively-oriented range, such as $$(-\infty, \infty)$$, versus a negatively oriented range like $$(\infty, -\infty)$$ (we assume that $$a \in I$$ though). Typically we just assume that all 1d integrals are over positively-oriented ranges unless otherwise specified, in which case we could simply omit the sign and write $$f(a)$$, but I'm trying to be careful now because it will help in higher dimensions.[^sign]

[^sign]: A slightly more sophisticated object would be something like $$\sgn_a(I)$$ which measures "the sign of $$a$$ in $$I$$" (which I have also [seen written]({% post_url 2019-02-23-exterior-6 %}) as $$a \diamond I$$). After all, the integral is actually $$0$$ if the range does not contain this point. But I figured there's enough funny business being explained right now, no need to add more.

The actual integration step is supposed to be easy once the integrand is proportional to $$1_a \widehat{dx}$$. The $$1_a$$ reduces the integral to a single point, while the $$\widehat{dx}$$ integrates out to give the sign of the integration range at that point.

Some pseudo-theoretical justification: Often we implement integration with Riemann summation, which decomposes the integration range into a bunch of oriented cells, each of which is described by a tangent vector $$\b{v}_i$$ (which in 1d is often simplified to $$x_{i+1} - x_i$$). Then we evaluate $$f \d x$$ on each of those tangent vectors and add up the result. Under a limit this converges (for a certain class of functions) to a definition value of the integral. We write this as $$\int_I f \d x = \lim \sum_{i \in I} f(x) \d x (\b{v}_i)$$, where the limit takes the number of partitions to infinity. 
...
Also, it is a more intuitive than the typical "nascent delta function" constructions. Typically delta functions are defined as either:
...
But this doesn't work! The problem is: what if we have something like $$\delta(x) \delta(x+y)$$? In an integrand this should evaluate at the point that satisfies $$x=0$$ and $$x+y=0$$, so at $$x=y=0$$. But $$\| d(x+y) \| = \sqrt{2}$$. More generally, it is well-known (although somewhat hard to find) that the change-of-variables formula for a multivariable delta function
...
We can also integrate a 2d delta function in $$\bb{R}^3$$. These set turn, but not all, of the terms in the differential into a unit differential.
...
And this seems like it will integrate to give something like:

$$\int_V \delta(g(\b{x})) f(\b{x}) \d^3 \b{x} = \sgn(V_g) \int_{g^{-1}(0)} f [\star \widehat{dg}]$$

Where $$\star \widehat{dg}$$ is the two-form which is the Hodge dual of $$\widehat{dg}$$. No idea how to do that integral, though.
...
What about products of more implicit functions?
