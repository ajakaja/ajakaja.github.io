...
Nor does an XML syntax limit you to declarative programming. You can implement control-flow in XML as well: just create tags like `<if>` and `<else>` and implement them appropriately in the XML engine. Then you can just define the entire syntax tree of your arbitrary imperative code as XML entities, like Java's ANT build system regrettably [tried](https://www.defmacro.org/ramblings/lisp.html).
...
The thing HTML didn't have but always needed was the ability to abstract nodes out, like this:
...
But it doesn't scale. If you've ever worked on an app built this way you know how it degenerates into illegibility.
