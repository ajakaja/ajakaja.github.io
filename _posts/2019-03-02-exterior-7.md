...
This is closely related to [Heron's Formula](https://en.wikipedia.org/wiki/Cayley%E2%80%93Menger_determinant) using the semiperimeter $$s = \frac{A + B + C}{2}$$: $$area(abc) = \sqrt{s (s-A) (s-B) (s-C)}$$. The transformation to get there isn't quite intuitive (try it!), but at leaset it's clear where you would start to generalize Heron's Formula to higher dimensions.
