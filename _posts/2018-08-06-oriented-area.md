...
{% include image.html filename="oriented_area/output/01-polygon.svg" width="200px" %}
...
{% include image.html filename="oriented_area/output/02-shoelace.svg" width="150px" %}
...
{% include image.html filename="oriented_area/output/08-angles.svg" width="250px" %}
...
{% include image.html filename="oriented_area/output/09-angle-addition.svg" width="300px" %}
...
{% include image.html filename="oriented_area/output/10-displacements.svg" width="150px" %}
...
{% include image.html filename="oriented_area/output/11-displacements2.svg" width="150px" %}
...
## 4. The Shoelace Formula with angles
...
{% include image.html filename="oriented_area/output/12-interior.svg" width="150px" %}
...
{% include image.html filename="oriented_area/output/13-exterior.svg" width="150px" %}
...
{% include image.html filename="oriented_area/output/14-complete.svg" width="250px" %}
...
{% include image.html filename="oriented_area/output/15-regular.svg" width="200px" %}
...
For a simple, positively-oriented polygon, the exterior angles add up to exactly $$2 \pi$$ radians. For non-simple polygons they may add up to any multiple of $$2 \pi$$ depending on how many clockwise or counterclockwise loops there are (that is, they add up to $$2 \pi$$ times the [turning number](https://en.wikipedia.org/wiki/Winding_number#Turning_number) of the curve):

{% include image.html filename="oriented_area/output/16-exterior2.svg" width="600px" caption="The first figure has $$\sum \theta_i = 2\pi$$, the second has $$\sum \theta_i = -2 \pi$$, and the third has $$\sum \theta_i = 0$$."%}
...
{% include image.html filename="oriented_area/output/17-exterior3.svg" width="125px" %}
...
2. Oriented triangles have oriented areas.
	...
	* Consider a triangle $$ABC$$. If $$A'$$ is a point on the line segment $$BC$$, then $$\text{area}(ABC) = \text{area}(A'AB) + \text{area}(A'CA)$$.<br/>{% include image.html filename="oriented_area/output/18-triangle.svg" width="150px" %}
	* If we allow ourselves _oriented_ triangles and areas, this remains true even if $$A'$$ is a point on the line $$\overrightarrow{BC}$$, but _outside_ the triangle:<br/>{% include image.html filename="oriented_area/output/19-triangle2.svg" width="200px" %}
...
## 5. The Shoelace Formula as an Area Integral

It's worth discussing how the shoelace formula is related to integral calculus. After all, if anything is the 'canonical' way to calculate area, it's an area integral: 

$$\text{area}(P) = \iint_P 1 \d x \d y = \iint_{P} 1 \d A$$

After all, an area integral says "the total area is what you get if you add up all the little bits of area". Makes sense.

[Green's theorem](https://en.wikipedia.org/wiki/Green%27s_theorem) says how we can translate an area integral over a region into an integral over its boundary. Specifically, it says that an area integral over a region is equivalent to a certain line integral around the boundary of the region. If $$L$$ and $$M$$ are functions with continuous partial derivatives in a region $$D$$ bounded by an (oriented) curve $$C$$, then:

$$\oint_C L \, dx + M \, dy = \iint_D [ \p_x M - \p_y L ] \d x \d y$$

For the simple case of $$\iint_D 1 \, \d x \d y$$, we just need to find any $$L,M$$ which have $$\p_x M - \p_y L = 1$$. This is easily done with either $$M(x,y) = x$$, $$L(x,y) = -y$$, or any combination of them, like $$\frac{1}{2} (M + L)$$. Therefore each of these gives an integral for area:

$$\begin{aligned} \text{area}(D) &= \iint_D 1 \, \d x \d y \\
&= \oint_C x \d y \\
&= \oint_C -y \d x \\
...
{% include image.html filename="oriented_area/output/21-integral.svg" width="200px" %}
...
While we're at it, maybe we can come up with an 'integral form' of (3) or (4). It should look something like this, right?
...
The fact that the shoelace formula is an implementation of Green's theorem means that there should be a way to generalize it to more dimensions. Green's theorem readily generalizes to [Stoke's Theorem](https://en.wikipedia.org/wiki/Stokes%27_theorem) in $$\bb{R}^n$$, which says that the integral of a function (/differential form) over a closed surface $$S$$ can be equated to the integral of its derivative through the enclosed volume $$V$$:
...
**Area of a figure in 3d**

Suppose we want to compute the area of this triangle $$T = (t_1,0,0), (0,t_2,0), (0,0,t_3)$$ which is floating in space $$\in \bb{R}^3$$:

{% include image.html filename="oriented_area/output/22-triangle.svg" width="350px" %}

We might naively try to compute the answer using the shoelace formula in the usual for $$A = \frac{1}{2} \sum_{i=0}^{n-1} p_i \times p_{i+1}$$:
...
Instead we can just talk about how things are oriented _relative_ to each other. A figure on the $$XY$$ plane is either oriented in the $$\b{z}$$ direction or the $$-\b{z}$$ direction. A figure in a different plane is oriented relative to that plane. In 2D we get to say that the orientation is just positive or negative because there is only one plane so we just compare everything to that. In 3D, though, _volumes_ have an absolute concept of orientation, but in 4D they would not-- they would just end up oriented to the, say, $$+\b{w}$$ or $$-\b{w}$$ axes!

The shoelace formula used on a general polygon in 3D will work the same way, giving a vector. Note that this does require the polygon be planar; that is, it needs to actually be a polygon and not some twisty non-planar set of points. I don't know if there's any meaning to the result if it's not.[^planar]

[^planar]: To tell if a set of points $$\in \bb{R}^3$$ are coplanar: find three points $$(a,b,c)$$ from the set that are not colinear and compute $$\b{q} = (b-a)\times (c-a)$$. This defines a plane normal to those points. Then compute all the dislacements to one of those points: $$\b{v}_i = p_i - a$$. They need to all lay in the same plane, so $$\b{v}_i \times \b{q}$$ should equal $$0$$ for all $$i$$.
